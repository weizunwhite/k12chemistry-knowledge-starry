...
const CLUSTER_CENTERS = {
  matter:       { cx: 300, cy: 250 },
  reaction:     { cx: 760, cy: 210 },
  substances:   { cx: 1130, cy: 300 },
  quantitative: { cx: 420, cy: 690 },
  experiment:   { cx: 1000, cy: 690 },
};
...
(() => {
  ...
  const byTheme = {};
  window.CHEM_NODES.forEach(n => { (byTheme[n.theme] = byTheme[n.theme] || []).push(n); });
...
const STAR_LINKS = window.CHEM_EDGES.filter(e => {
  const a = window.CHEM_NODE_BY_ID[e.from], b = window.CHEM_NODE_BY_ID[e.to];
  return a && b && a.theme === b.theme && STAR_POS[e.from] && STAR_POS[e.to];
});
const STAR_CROSS = window.CHEM_EDGES.filter(e => {
  const a = window.CHEM_NODE_BY_ID[e.from], b = window.CHEM_NODE_BY_ID[e.to];
  return a && b && a.theme !== b.theme && STAR_POS[e.from] && STAR_POS[e.to];
});
...
const STAR_SIG = {};
window.CHEM_NODES.forEach(n => { STAR_SIG[n.id] = 0; });
window.CHEM_EDGES.forEach(e => {
...
//   pillar  枢纽 · 最关键的化学枢纽 (力、能量、欧姆定律…)  ★ 4-point flare + huge halo
...
const TWINKLE_PHASE = {};
window.CHEM_NODES.forEach((n, i) => { TWINKLE_PHASE[n.id] = (i * 0.37) % 1; });
...
function ConstellationVariation({ onSelectNode, selectedId, mastered, searchQuery, themeFilter, onToggleMastery, hideHeader, recommended, stageFilter = 'junior' }) {
  ...
  const renderEdge = (e, isCross) => {
    ...
    const theme = window.CHEM_THEMES[fromNode.theme];
    // Edge styling
    ...
    <div className="con-root">
      ...
      <div className="con-header">
        <div>
          ...
          <h1 className="con-title">初中化学 · 知识星空</h1>
      ...
      <div className="con-mapWrap" ref={mapWrapRef}
        ...
        <div className={'con-hero' + (focused ? ' con-hero-hide' : '')}>
          <div className="con-heroTitle">化学是研究物质组成、结构、变化的科学</div>
        </div>
        ...
        {focused && lineage && (
          <div className="con-focusBadge">
            <div className="con-focusName">{window.CHEM_NODE_BY_ID[focused].name}</div>
