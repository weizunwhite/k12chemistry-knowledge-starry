...
function NodeDetail({ node, onClose, onJump, mastered, onToggleMastery }) {
  const theme = window.CHEM_THEMES[node.theme];
  const history = (window.CHEM_HISTORY || {})[node.id];
  const lesson = (window.CHEM_LESSON || {})[node.id];
  ...
  const readSavedSize = () => {
    try {
      const v = JSON.parse(localStorage.getItem('chemistry-panel-size') || 'null');
  ...
  useEffect(() => {
    if (!panelRect) return;
    try {
      localStorage.setItem('chemistry-panel-size',
  ...
  return (
    ...
    >
      ...
      <div className="nd-header" style={{ borderColor: theme.soft }} onPointerDown={startPanelDrag}>
        ...
        {window.CHEM_NODE_GRADE && window.CHEM_NODE_GRADE[node.id] && (
          <span className="nd-gradePill">{window.CHEM_NODE_GRADE[node.id]}</span>
        )}
      ...
      <div className="nd-body" ref={scrollRef}>
        {tab === 'overview' && (
          <div className="nd-overview">
            ...
            {(lesson?.figure || (window.CHEM_FIGURE_BANK || {})[node.id]) && (
              <section className="nd-section">
                <div className="nd-sectionLabel">图示</div>
                <div className="nd-figureBox" dangerouslySetInnerHTML={{ __html: lesson?.figure || window.CHEM_FIGURE_BANK[node.id] }} />
              </section>
            )}

            {window.NodeWidget && <window.NodeWidget nodeId={node.id} theme={theme} />}

            {(window.CHEM_SUBNODES || {})[node.id] && (
              <section className="nd-section">
                <div className="nd-sectionLabel">细分知识点 · 逐项展开</div>
                <div className="sn-list">
                  {window.CHEM_SUBNODES[node.id].map((sn, i) => (
                ...
                <div className="nd-reasons">
                  {prereqs.length > 0 && (
                    <div className="nd-reasonGroup">
                      ...
                      {prereqs.map(p => {
                        const why = window.CHEM_EDGE_WHY[`${p.id}→${node.id}`];
                        const pTheme = window.CHEM_THEMES[p.theme];
                        return (
                  ...
                  {leadsTo.length > 0 && (
                    <div className="nd-reasonGroup">
                      ...
                      {leadsTo.map(p => {
                        const why = window.CHEM_EDGE_WHY[`${node.id}→${p.id}`];
                        const pTheme = window.CHEM_THEMES[p.theme];
                        return (
                          <button key={p.id} className="nd-reasonCard" onClick={() => onJump(p.id)}>
                            <div className="nd-reasonCardHead">
                ...
                <div className="nd-chipRow">
                  {connections.map(p => (
                    <button key={p.id} className="nd-chip" onClick={() => onJump(p.id)}
                      style={{ borderColor: window.CHEM_THEMES[p.theme].soft, color: window.CHEM_THEMES[p.theme].deep }}>
                      <span className="nd-dot" style={{ background: window.CHEM_THEMES[p.theme].color }}></span>
                      {p.name}
                    </button>
                  ))}
                </div>
