...
const MISTAKE_KEY = 'chemistry-mistakes';
...
function NodeQuiz({ node, theme }) {
  ...
  const seed = (window.CHEM_QUIZ_SEED || {})[node.id];
  ...
  const generate = async () => {
    ...
    const prompt = `你是初中化学命题专家。请就「${node.name}」这个知识点出 3 道单选题，难度依次：基础 / 中等 / 进阶。${ctx}
      ...
      <div className="mb-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-body">
          ...
          {mistakes.length === 0 ? (
            ...
            const node = window.CHEM_NODE_BY_ID[nodeId];
            const theme = node ? window.CHEM_THEMES[node.theme] : null;
            return (
